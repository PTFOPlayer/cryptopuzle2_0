...
fn main() {
    ...
    println!("size:{}, U:{}, max:{}, sum:{}\n{:?}", size, bu, max, sum, public.clone());

    let pairs = (1..bu+1).map(|i|{
        let mut vec = vec![];
        for j in i+1..bu+1 {
    ...
    let pairs = pairs.into_iter().flatten().unique().collect::<Vec<(i128,i128)>>();
    println!("workable pairs created: {}", pairs.len());

    let options = pairs.par_iter().map(|t|{
        let (i,j) = t;
        let mut breaks = false;
        for n in max..i128::pow(2, 63) {
            for q in bu..n {
                if euc_lib::I128::euc(q,n) == 1 && public[0] == (i*q)%n && public[1] == (j*q)%n {
                    return (*i,*j, q,n);
                }
                if n > sum {
                    breaks = true;
    ...
    let options = options.iter().unique().collect::<Vec<&(i128,i128,i128,i128)>>();
    println!("working options created, {}, {:?}", options.len(), options);
    
    let estim = options.par_iter().map(|t| {
        let (_, _, q, n) = t;
        let mut count = 0;
        let mut prev = 1;
    ...
    let mut estim = estim.iter().unique().sorted().collect::<Vec<&(i128,i128)>>();
    estim.sort();
